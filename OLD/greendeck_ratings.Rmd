...
```{r}
gd <- read.csv("C:/Users/aaron/Desktop/Colab Notebooks/GreenDeckReseach_dailymeans.csv")
View(gd)
gd$Timestamp = mdy(gd$Timestamp)

```

```{r}
gdm <- melt(gd, id.vars = 'Timestamp')
gdm.overall <- dplyr::filter(gdm, grepl('overall', variable))
gd.means <- gdm %>% group_by(variable) %>% summarise(mean = mean(value,na.rm = TRUE))
...
plants <- c('am','as','bg','cp','eu','ec','fc','fi','mb','mv','pm','sb') #TODO: MAKE ALL VARIABLE NAMES UNIQUE IN ORIGINAL DOCUMENT

gd.p <- setNames(as.list(rep(NA, length(plants))), plants) #create empty df with keys = plant names

df.sd <- as.data.frame(cbind(start_dates21,start_dates22))
row.names(df.sd) <- plants

df.ed <- as.data.frame(cbind(end_dates21,end_dates22))
row.names(df.ed) <- plants

for (i in plants) {
  gd.p[[i]] <- gdm[gdm$variable %like% i,]
}
